...
## Aim 
The BBN results extracted from GeNie are plotted and analyzed here. We also calculate means and medians of the two models (A describing diffuse sources, and B with the addition of point sources in the form of farmyards and septic tanks), and calculate the percentage bias.  
...
# Read csv of fitted to observed monthly TP
...
make same histogram for the model and then compare histograms 
use month, TRP, model columns. Also useful to scale by the number of counts or any other way to scale the model hist and the data hist so they look 
...
values are in the ave column
probabilities are in qtile column
...
```{r}
sampled_observedTRP<-sampleFit(myTRPfit, 10000) # this creates an data type called "double"
...
## Analyze Diffuse Ptool
Summarize simulations results
```{r}
names(diffuseconcentrations)[names(diffuseconcentrations) == 'ï..Pconcentration_mgL']<- 'Pconcentration_mgL'

summary(diffuseconcentrations)
quantile(diffuseconcentrations$Pconcentration_mgL, probs=c(0.05, 0.25, 0.5, 0.75))
```
...
## Analyze Ptool point and diffuse
...
quantile(global_ptool_conc$Pconcentration_mgL, probs=c(0.05, 0.25, 0.5, 0.75))
```
...
## Goodness of fit 
# Compare GeNie concentrations (diffuse P model) with fitted TP from observed. This is possible because we previously produced the `lognormal_observed_TP` & `lognormal_observed_TRP` data.
...
# PBIAS Model A
```{r}
sim<-filtered_diffuse$diffuse_ptool # filtered GeNie simulations
obs<-sample(lognormal_observed_TRP,7229) # observed fitted to lognormal
```
...
# PBIAS Model B
```{r}
sim2<-filtered_global$global_ptool # GeNie simulations global model
obs2<-sample(lognormal_observed_TRP,7851) # observed fitted to lognormal
```

```{r}
pbias(sim2, obs2, na.rm=F) # global model
```
...
# PBIAS of every month - Model A (filter the diffuse concentrations first)
...
Recall that the csv of fitted to observed monthly TRP is called `sampled_observedTRP`, while `global_ptool_conc_months` is the predicted TP from the BBN
```{r}
# monthly data
global_ptool_conc_months<-read.csv("results/20230920_Ptool_v4_months.csv", header=T)
names(global_ptool_conc_months)[names(global_ptool_conc_months) == 'ï..January']<- 'January'
...
# Make labels and prepare df for figures
```{r}
...
merge two dataframes in one, marking clearly what is observed and what is predicted. Recall that the csv of fitted to observed monthly TP is called `sampled_observedTP`, while `global_ptool_conc_months` is the predicted TP from the BBN
```{r}
...
## Compare predictions to actual monthly observations (months= columns of both df)
